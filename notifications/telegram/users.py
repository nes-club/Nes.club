...
def notify_user_profile_approved(user):
    ...
    if user.telegram_id:
        send_telegram_message(
            ...
            text=f"🚀 Поздравляем, вы прошли модерацию. Добро пожаловать в сообщество!"
...
def notify_user_ping(user, message):
    if user.telegram_id:
        send_telegram_message(
            chat=Chat(id=user.telegram_id),
            text=f"👋 <b>Вам письмо от модераторов сообщества:</b> {message}"
        )
...
def notify_user_auth(user, code):
    if user.telegram_id:
        send_telegram_message(
            chat=Chat(id=user.telegram_id),
            text=f"<code>{code.code}</code> — ваш одноразовый код для входа в сообщество",
        )
